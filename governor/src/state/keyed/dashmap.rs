...
pub type DashMapStateStore<K, S = std::collections::hash_map::RandomState> =
    DashMap<K, InMemoryState, S>;

impl<K: Hash + Eq + Clone, S: std::hash::BuildHasher + Clone> StateStore
...
impl<K: Hash + Eq + Clone, S: std::hash::BuildHasher + Clone> ShrinkableKeyedStateStore<K>
    for DashMapStateStore<K, S>
{
