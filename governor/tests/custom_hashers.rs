...
macro_rules! test_rate_limiter {
    ($map_method:ident, $hasher_type:ty, $test_name:ident) => {
        ...
        fn $test_name() {
            ...
            let lb =
                RateLimiter::$map_method(Quota::per_second(nonzero!(2u32)), clock.clone(), hasher);

            // Create two distinct keys
            let key1 = 1u32;
            let key2 = 2u32;

            // Check that rate limiting works correctly for key1
            assert_eq!(Ok(()), lb.check_key(&key1)); // First check
            assert_eq!(Ok(()), lb.check_key(&key1)); // Second check
            assert_ne!(Ok(()), lb.check_key(&key1)); // Third check (should fail)

            // Key2 should be unaffected and have full capacity
            assert_eq!(Ok(()), lb.check_key(&key2)); // First check
            assert_eq!(Ok(()), lb.check_key(&key2)); // Second check
            assert_ne!(Ok(()), lb.check_key(&key2)); // Third check (should fail)

            // After waiting, both keys should work again
            clock.advance(Duration::from_secs(1));
            assert_eq!(Ok(()), lb.check_key(&key1));
            assert_eq!(Ok(()), lb.check_key(&key2));
        }
    };
}
